...
itr = 0
w_itr = 0
#print(A.shape)      # 1 x 3  --> Should be 1 x 3
#print(x_cur.shape)  # 3 x 1
#print(b.shape)      #  1 x 1
...
while(w_itr < 100):
    ...
    ## DO WE NEED TO DO THIS AGAIN OR JUST USE Q_TOTAL?

    Q2 , Q2_alt, index2 = qubo_prep_adaptive(A,b,k,scale_list,offset_list,bits_no,varnames=varnames)
    #sampleset = dimod.ExactSolver().sample_qubo(Q)  

    #sampler = tabu.TabuSampler()
    sampler = dimod.ExactSolver()
    #sampleset = sampler.sample_qubo(Q_alt2, timeout=60000)
    sampleset = sampler.sample_qubo(Q2_alt)
    ...
    soln_dict = convert_result(sampleset.first.sample, index2)
